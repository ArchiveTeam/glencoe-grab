...
LUA_SHA1 = get_hash(os.path.join(CWD, 'glencoe.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        wget_args = [
            ...
            '--timeout', '30',
            '--tries', 'inf',
            '--domains', 'glencoe.mheducation.com',
            '--span-hosts',
            '--waitretry', '30',
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'c':
                if '-' in item_value:
                    ...
                    ids = [str(i).zfill(10) for i in range(int(start), int(stop)+1)]
                    ids += [s[:-1]+'x' for s in ids]
                    ids = sorted(set(ids))
                else:
                    ids = [item_value]
                for i in ids:
                    wget_args.extend(['--warc-header', 'glencoe-center: '+str(i)])
                    wget_args.append('https://glencoe.mheducation.com/sites/'+str(i)+'/')
            else:
...
project = Project(
    ...
    project_html='''
        ...
        <h2>glencoe.mheducation.com <span class="links"><a href="https://glencoe.mheducation.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/glencoe/">Leaderboard</a></span></h2>
        <p>Archiving Glencoe online learning centers.</p>
    '''
)
